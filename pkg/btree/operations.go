...
func shouldMerge(tree *BTree, node BNode, idx uint16, updated BNode) (int, BNode) {
	if updated.nbytes() > constants.PageSize/4 {
		return 0, BNode{}
	}

	if idx > 0 {
		sibling := tree.get(node.getPtr(idx - 1))
		merged := sibling.nbytes() + updated.nbytes() - constants.HeaderSize
		if merged <= constants.PageSize {
			return -1, sibling
		}
	}
...
func nodeDelete(tree *BTree, node BNode, idx uint16, key []byte) BNode {
	...
	new := NewBNode(make([]byte, constants.PageSize))

	// get merge direction - either left or right sibling
	mergeDir, sibling := shouldMerge(tree, node, idx, updated)
	switch {
	case mergeDir < 0: // if left
		merged := NewBNode(make([]byte, constants.PageSize)) // prepare new node to merge old into
	...
	case mergeDir == 0:
		if updated.nkeys() == 0 { // parent only has one child, child is empty after deletion
			...
			new.setHeader(NodeTypeInternal, 0)
		} else {
